...
const sendWithBrevoAPI = async (to, subject, htmlContent, from = { email: 'updates.codecatalyst@gmail.com', name: 'AetherTrack' }) => {
  try {
...
export const sendVerificationEmail = async (fullName, email, verificationCode, password, workspaceName) => {
  try {
    ...
    const subject = '✉️ Verify Your AetherTrack Account - Action Required';
  ...
  <div class="email-wrapper">
    <div class="container">
      ...
      <div class="content">
        ...
        <div class="message">
          <p>Welcome to <strong>AetherTrack</strong>! 🎉</p>
        ...
        <div style="margin-top: 40px; padding: 25px; background: #f8fafc; border-radius: 12px; text-align: center;">
          <p style="color: #64748b; margin: 0; font-size: 14px;">Didn't create a AetherTrack account? Please ignore this email or contact support if you have concerns.</p>
        </div>
      </div>

      <div class="footer">
        <p><strong>AetherTrack</strong> - Collaborative Task Management</p>
        <p style="font-size: 12px; opacity: 0.8; margin-top: 15px;">This is an automated message. Please do not reply.</p>
        <p style="margin-top: 15px;"><a href="${appUrl}" style="color: #90cdf4; text-decoration: none;">Visit AetherTrack</a></p>
      </div>
    </div>
  </div>
</body>
...
export const sendPasswordResetEmail = async (fullName, email, newPassword) => {
  try {
    ...
    const subject = '🔑 AetherTrack - Password Reset';
    const htmlContent = `
<!DOCTYPE html>
<html lang="en">
<head>
...
export const sendEmail = async (to, subject, htmlContent, from = { email: 'updates.codecatalyst@gmail.com', name: 'AetherTrack' }) => {
  try {
    // Try Brevo API first (preferred)
    if (process.env.BREVO_API_KEY) {
...
export const sendPasswordResetLink = async (full_name, email, resetToken) => {
  try {
    ...
    if (process.env.BREVO_API_KEY) {
      const result = await sendWithBrevoAPI(email, 'Reset Your Password - AetherTrack', htmlContent);
      if (result.success) {
    ...
    const transporter = createTransporter();
    const mailOptions = {
      from: `"AetherTrack" <${process.env.EMAIL_FROM || process.env.EMAIL_USER}>`,
      to: email,
      subject: 'Reset Your Password - AetherTrack',
      html: htmlContent
    };

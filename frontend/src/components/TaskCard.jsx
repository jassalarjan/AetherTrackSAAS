  ...
  return (
    ...
    >
      ...
      <div className={`absolute top-0 left-0 right-0 h-1 ${
        task.priority === 'urgent' ? 'bg-gradient-to-r from-red-600 via-red-500 to-red-600' :
      ...
      <div className="p-5" onClick={onView}>
        ...
        <div className="flex items-start justify-between mb-4 cursor-pointer">
          <div className="flex-1 min-w-0 pr-3">
            <h3 className="text-lg font-bold text-white mb-2 line-clamp-2 group-hover:text-[#136dec] transition-colors duration-200">
              {task.title}
            </h3>
            {task.description && (
              <p className="text-sm text-[#9da8b9] line-clamp-2 leading-relaxed">
                {task.description}
              </p>
            )}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-4">
          ...
          <div className="relative" onClick={(e) => e.stopPropagation()}>
            ...
            {showStatusMenu && (
              <div className="absolute top-full left-0 right-0 mt-2 bg-[#1c2027] border border-[#282f39] rounded-lg shadow-2xl overflow-hidden z-10 animate-slideUp">
                  ...
                  <button
                    ...
                    className={`w-full px-3 py-2.5 text-left text-sm transition-colors ${
                      task.status === status 
                        ...
                        : 'text-[#9da8b9] hover:bg-[#282f39] hover:text-white'
                    }`}
                  >
            ...
            <div className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${
              isOverdue 
                ...
                : daysUntilDue <= 3 
                  ...
                  : 'bg-[#282f39] text-[#9da8b9] border-[#3a4454]'
        ...
        {task.assigned_to && task.assigned_to.length > 0 && (
          <div className="flex items-center gap-3 px-3 py-2.5 bg-[#282f39]/30 rounded-lg mb-4 border border-[#282f39] transition-all duration-200 hover:bg-[#282f39]/50">
            <User size={16} className="text-[#9da8b9] flex-shrink-0" />
            <div className="flex items-center gap-2 flex-1 min-w-0">
              <div className="size-8 rounded-full bg-gradient-to-br from-purple-500 via-blue-500 to-indigo-600 flex items-center justify-center text-xs text-white font-bold shadow-lg ring-2 ring-[#282f39]">
        ...
        {/* Actions */}
        <div className="flex items-center gap-2 pt-4 border-t border-[#282f39]" onClick={(e) => e.stopPropagation()}>
          {canEdit && (
            <button
              onClick={onEdit}
          ...
          {canDelete && (
            <button
              onClick={onDelete}
              className="min-h-[44px] min-w-[44px] flex items-center justify-center px-3 py-2.5 bg-[#282f39] hover:bg-red-600 text-[#9da8b9] hover:text-white rounded-lg transition-all duration-200 hover:shadow-lg hover:shadow-red-900/30 hover:scale-[1.02] active:scale-[0.98]"
            >
              <Trash2 size={16} />
            </button>
          )}
          {!canEdit && !canDelete && (
            <button 
              onClick={onView}
              className="flex-1 min-h-[44px] flex items-center justify-center gap-2 px-4 py-2.5 bg-[#282f39] hover:bg-[#3a4454] text-white text-sm font-semibold rounded-lg transition-all duration-200 hover:shadow-lg hover:scale-[1.02] active:scale-[0.98]"
            >

 ...
 * @param {String} reportType - 'daily', 'weekly', 'monthly', 'all', or 'filter'
...
export const generateComprehensivePDFReport = (tasks, analyticsData, filters, user, reportType = 'all', projects = []) => {
  try {
    ...
    const filteredTasks = filterTasksByDateRange(tasks, reportType, filters);
    const filteredAnalytics = recalculateAnalytics(filteredTasks);
    
    let yPosition = 20;

    // ========== COVER PAGE ==========
    addCoverPage(doc, reportType, user, filteredTasks.length, filters);
    ...
    const summaryData = generateSummaryData(filteredAnalytics, reportType, filters);
    autoTable(doc, {
      startY: yPosition,
    ...
    // ========== FOOTER ON ALL PAGES ==========
    addFootersToAllPages(doc, reportType, filters);
    
    // ========== SAVE PDF ==========
    const timestamp = new Date().toISOString().slice(0, 10);
    const reportTypeLabel = getReportTypeFileLabel(reportType, filters);
...
function addCoverPage(doc, reportType, user, taskCount, filters) {
  const pageWidth = doc.internal.pageSize.getWidth();
  const pageHeight = doc.internal.pageSize.getHeight();
  ...
  doc.setFontSize(18);
  doc.setFont(undefined, 'normal');
  const reportLabel = getReportTypeTitle(reportType, filters);
  ...
  doc.text(`Report Period: ${getReportPeriodLabel(reportType, filters)}`, pageWidth / 2, 180, { align: 'center' });
...
function generateSummaryData(analytics, reportType, filters) {
  ...
  return [
    ...
    ['Report Period', getReportPeriodLabel(reportType, filters), 'Timeframe'],
  ];
}

function filterTasksByDateRange(tasks, reportType, filters) {
  if (reportType === 'filter') return tasks;
...
function addFootersToAllPages(doc, reportType, filters) {
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    // Footer text
    const reportLabel = getReportTypeFooterLabel(reportType, filters);
    doc.text(
...
function getReportPeriodLabel(reportType, filters = {}) {
  const now = new Date();
  switch (reportType) {
    case 'filter':
      switch (filters.dateRange) {
        ...
        case 'custom':
          if (filters.customStartDate && filters.customEndDate) {
            const start = new Date(filters.customStartDate).toLocaleDateString();
            const end = new Date(filters.customEndDate).toLocaleDateString();
            return `${start} - ${end}`;
          }
          return 'Custom Range (Dates Not Fully Selected)';
        default:
          return 'All Time (Filter Panel)';
      }
    case 'daily':
      return now.toLocaleDateString();
    case 'weekly':
...
function getReportTypeTitle(reportType, filters = {}) {
  if (reportType === 'filter') {
    const rangeName = filters.dateRange === 'today'
      ? 'Today'
      : filters.dateRange === 'week'
        ...
        : filters.dateRange === 'month'
          ...
          : filters.dateRange === 'custom'
            ? 'Custom Range'
            : 'All Time';
    return `Filter-Based Report (${rangeName})`;
  }
...
function getReportTypeFooterLabel(reportType, filters = {}) {
  if (reportType === 'filter') {
    const rangeName = filters.dateRange === 'today'
      ? 'Filtered-Today'
      : filters.dateRange === 'week'
  ...
  return reportType === 'all' ? 'Full' : reportType.charAt(0).toUpperCase() + reportType.slice(1);
}

function getReportTypeFileLabel(reportType, filters = {}) {
  if (reportType === 'filter') {
    const rangeKey = (filters.dateRange || 'all').toString().replace(/[^a-zA-Z0-9]+/g, '_');
    return `Filtered_${rangeKey}`;
  }
